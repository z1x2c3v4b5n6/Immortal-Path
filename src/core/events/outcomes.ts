import type { EventOutcome, EventRequirement, Player } from '../../models'
...
export function calculateOutcomeWeight(outcome: EventOutcome, player: Player): number {
  ...
  if (outcome.tags?.includes('danger')) multiplier *= Math.max(.2, 1 - originValue('adventureSafety'))
  return Math.max(0, outcome.weight * multiplier)
}

export function selectEventOutcome(outcomes: EventOutcome[], player: Player, rng: RandomService): EventOutcome {
  return rng.weightedRandom(outcomes.map((outcome) => ({ value: outcome, weight: calculateOutcomeWeight(outcome, player) })))
}
