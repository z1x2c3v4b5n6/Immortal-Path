...
import { allocateStat, availableTalents, randomizeStats, randomSpiritRoot, randomTalentIds, selectableOrigins, STAT_KEYS, STAT_LABELS, validateBuild } from '../core/creation/creation'
...
const remainingStats = ref(origin.value.freeStatPoints)
...
const talentBudget = computed(() => origin.value.talentPoints + (first.value ? 0 : game.state.reincarnation.selections.extraTalentPoints) + (talentMode.value === 'random' ? 1 : 0))
...
function resetStats() {
  ...
  remainingStats.value = origin.value.freeStatPoints
  if (statMode.value === 'random') Object.assign(stats, randomizeStats(origin.value, capBonus.value, random)), remainingStats.value = 0
}
...
function changeStat(key: StatKey, delta: number) {
  ...
  Object.assign(stats, result.stats); remainingStats.value = result.remaining
}
function randomStats() { statMode.value = 'random'; Object.assign(stats, randomizeStats(origin.value, capBonus.value, random)); remainingStats.value = 0 }
