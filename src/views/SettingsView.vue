...
async function importSave(event: Event) {
  ...
  if (!file) return
  try { game.replaceState(deserializeSave(await file.text())); await game.manualSave(); message.value = '存档已成功载入并升级至 V2。' }
