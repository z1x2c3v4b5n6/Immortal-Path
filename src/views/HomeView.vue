...
<template>
  ...
  <div v-else class="home-view">
    <section class="hero-card panel"><div><p class="eyebrow">{{ game.player.origin.name }} · {{ game.player.spiritRoot.name }}</p><h1>洞中无岁月，<br><i>世上已千年。</i></h1><p>{{ game.player.origin.description }} 此世已行至 {{ game.currentRealm.name }}，尚余约 {{ game.remainingYears }} 年天命。</p></div><div class="year-seal"><small>玄历</small><strong>{{ game.state.world.currentYear }}</strong><span>第 {{ game.player.generation }} 世</span></div></section>
